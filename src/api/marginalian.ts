...
async function fetchFeed(): Promise<Rss2JsonItem[]> {
  ...
  const url = encodeURIComponent('https://www.themarginalian.org/feed/');
  const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${url}&count=40`);
  if (!res.ok) throw new Error('Marginalian feed failed');
  const json: Rss2JsonResponse = await res.json();
  if (json.status !== 'ok' || !json.items?.length) throw new Error('No Marginalian items');

  _cache = { items: json.items, expires: Date.now() + 10 * 60 * 1000 };
  return _cache.items;
}

export async function fetchMarginalianArticle(
  interest: InterestCategory
): Promise<Article | null> {
  if (!interest.hasMarginalian) return null;
  ...
  try {
    ...
    return {
      id: `marginalian_${djb2(item.link || item.guid)}`,
      title: item.title,
      description: 'The Marginalian',
      extract,
      pageUrl: item.link,
      wikiTitle: '',
      ...
      source: 'marginalian',
      author: item.author || 'Maria Popova',
