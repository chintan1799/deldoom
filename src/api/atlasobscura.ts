...
async function fetchFeed(): Promise<Rss2JsonItem[]> {
  ...
  const url = encodeURIComponent('https://www.atlasobscura.com/feeds/latest');
  const res = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${url}&count=40`);
  if (!res.ok) throw new Error('Atlas Obscura feed failed');
  const json: Rss2JsonResponse = await res.json();
  if (json.status !== 'ok' || !json.items?.length) throw new Error('No Atlas Obscura items');

  _cache = { items: json.items, expires: Date.now() + 10 * 60 * 1000 };
  return _cache.items;
}

export async function fetchAtlasObscuraArticle(
  interest: InterestCategory
): Promise<Article | null> {
  if (!interest.hasAtlasObscura) return null;
  ...
  try {
    ...
    return {
      id: `atlasobscura_${djb2(item.link || item.guid)}`,
      title: item.title,
      ...
      source: 'atlasobscura',
      author: item.author || 'Atlas Obscura',
