...
function BackCardPeek({ article }: { article: Article }) {
  const imageUrl = article.thumbnailUrl || article.imageUrl;
  return (
    <div className="fixed inset-0">
      {imageUrl ? (
        <img src={imageUrl} alt="" className="w-full h-[48vh] object-cover" />
      ) : (
        <div
      ...
      <div className="absolute inset-x-0 top-[36vh] bg-white dark:bg-navy-950 rounded-t-[28px] px-5 pt-4">
        <h2 className="text-2xl font-black text-navy-900 dark:text-white leading-tight truncate opacity-60">
          {article.title}
        </h2>
      </div>
    </div>
...
export function HomeScreen() {
  ...
  const { article, nextArticle, loading, error, advance } =
    useArticleQueue(selectedInterests);
  ...
  // Scroll mode — hand off entirely to ScrollFeed
  if (feedMode === 'scroll') {
    return (
      <>
        <ScrollFeed />
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 dark:bg-navy-950">
      ...
      {nextArticle && !loading && (
        ...
        >
          <BackCardPeek article={nextArticle} />
        </div>
      )}
